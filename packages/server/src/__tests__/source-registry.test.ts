/**
 * Tests for the server source registry.
 *
...
describe("createSourcesWithCategory", () => {
  ...
  it("pairs each source with its category", () => {
    const results = createSourcesWithCategory(["books"])
    for (const r of results) {
      expect(r.category).toBe("books")
      expect(r.source).toHaveProperty("name")
